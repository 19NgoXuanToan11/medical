...
public class InjectionFormService : IInjectionFormService
{
    ...
    public async Task<bool> UpdateInjectionFormAsync(InjectionForm injectionForm)
    {
        try
        {
            ...
            {
                _logger.LogWarning("InjectionForm with ID {FormId} not found", injectionForm.FormId);
                return false;
            }
            ...
            if (injectionForm.StudentId.HasValue && injectionForm.StudentId.Value > 0)
            {
                ...
                if (student == null)
                {
                    _logger.LogWarning("Student with ID {StudentId} not found", injectionForm.StudentId.Value);
                    throw new InvalidOperationException("Student not found");
                }
            }
            ...
            if (injectionForm.ParentId.HasValue && injectionForm.ParentId.Value > 0)
            {
                ...
                if (parent == null)
                {
                    _logger.LogWarning("Parent with ID {ParentId} not found", injectionForm.ParentId.Value);
                    throw new InvalidOperationException("Parent not found");
                }
            }

            // Validate status only if ConsentStatus is provided
            if (!string.IsNullOrEmpty(injectionForm.ConsentStatus) && !IsValidConsentStatus(injectionForm.ConsentStatus))
            {
                _logger.LogWarning("Invalid consent status: {ConsentStatus}", injectionForm.ConsentStatus);
                throw new InvalidOperationException("Invalid consent status value");
            }
        ...
        {
            _logger.LogError(ex, "Error updating injection form with ID {FormId}", injectionForm.FormId);
            throw;
        }
    }
    ...
    public async Task<InjectionForm?> CreateVaccinationScheduleAsync(InjectionForm schedule)
    {
        try
        {
            ...
            try
            {
                gradeIds = JsonSerializer.Deserialize<List<string>>(schedule.GradeIds) ?? new List<string>();
            }
            catch
            {
            ...
            foreach (var gradeId in gradeIds)
            {
                ...
                if (gradeId.StartsWith("grade-") && int.TryParse(gradeId.Substring(6), out int gradeLevel))
                {
                    ...
                    foreach (var classItem in classes)
                    {
                        ...
                        var studentsInClass = await _classRepository.GetStudentsByClassIdAsync(classItem.ClassId);
                        ...
                        foreach (var student in studentsInClass)
                        {
                            ...
                            var healthProfile = await _healthProfileRepository.GetHealthProfileByStudentCodeAsync(student.StudentCode);
                            if (healthProfile != null)
                            {
                                allHealthProfiles.Add(new
                                {
            ...
            if (schedule.ScheduledDate.HasValue && schedule.StartTime.HasValue && !string.IsNullOrEmpty(schedule.Location))
            {
                ...
                if (hasConflict)
                {
                    throw new InvalidOperationException("A schedule already exists at this time and location");
                }
            }

            // Create the schedule
            var createdSchedule = await _injectionFormRepository.CreateInjectionFormAsync(schedule);
            _logger.LogInformation("Created vaccination schedule with ID {ScheduleId} for {StudentCount} students across {ClassCount} classes", 
                createdSchedule.FormId, totalStudentCount, allClasses.Count);
    ...
    public async Task<bool> UpdateVaccinationScheduleAsync(InjectionForm schedule)
    {
        try
        {
            ...
            var existingSchedule = await _injectionFormRepository.GetVaccinationScheduleByIdAsync(schedule.FormId);
            if (existingSchedule == null)
            {
                throw new InvalidOperationException("Schedule not found or is not a vaccination schedule");
            }
            ...
            if (schedule.ScheduledDate != existingSchedule.ScheduledDate ||
            ...
            if (success)
            {
                _logger.LogInformation("Updated vaccination schedule with ID {ScheduleId}", schedule.FormId);
            }
            else
            {
                _logger.LogWarning("Failed to update vaccination schedule with ID {ScheduleId}", schedule.FormId);
            }
        ...
        {
            _logger.LogError(ex, "Error updating vaccination schedule with ID {ScheduleId}", schedule.FormId);
            throw;
        }
    }
    ...
    public async Task<bool> DeleteVaccinationScheduleAsync(int id)
    {
        try
        {
            ...
            {
                _logger.LogWarning("Failed to delete vaccination schedule with ID {ScheduleId}", id);
            }
    ...
    {
        return await _injectionFormRepository.GetVaccinationSchedulesByDateRangeAsync(startDate, endDate);
    }
    ...
    {
        return await _injectionFormRepository.HasScheduleConflictAsync(scheduledDate, startTime, location);
    }
} 