...
public class StudentRepository : IStudentRepository
{
    ...
    )
    {
        var students = await _context
            .Students.Where(s => studentIds.Contains(s.StudentId))
            .Include(s => s.HealthProfiles)
            .Include(s => s.InjectionForms)
        ...
        foreach (var student in students)
        {
            ...
            if (
                profile != null
                && profile.HasChronicDiseases == true
                && !string.IsNullOrEmpty(profile.ChronicDetails)
            )
                continue;
            ...
            if (
                ...
                && !string.IsNullOrEmpty(profile.AllergyDetails)
            )
                continue;
            // 3. Đang mắc bệnh cấp tính/đang sốt (dựa vào HealthCheckResult gần nhất)
            var lastCheck = await _context
                .HealthCheckResults.Where(r => r.StudentId == student.StudentId)
                .OrderByDescending(r => r.ExaminedDate)
                .FirstOrDefaultAsync();
            if (
                lastCheck != null
                && !string.IsNullOrEmpty(lastCheck.GeneralFindings)
                && lastCheck.GeneralFindings.ToLower().Contains("sốt")
            )
                continue;
            // 4. Đã tiêm vaccine này gần đây (giả sử chống chỉ định 180 ngày)
            var lastInjection = student
                .InjectionForms.Where(f =>
                    f.VaccineId == vaccineId && f.ConsentStatus == "Approved"
                )
            ...
            if (
                lastInjection != null
                && (injectionDate - lastInjection.AdministeredDate).TotalDays < 180
            )
                continue;
            ...
            if (
                profile != null
                && profile.HasPreviousTreatment == true
                && !string.IsNullOrEmpty(profile.TreatmentDetails)
            )
                continue;
