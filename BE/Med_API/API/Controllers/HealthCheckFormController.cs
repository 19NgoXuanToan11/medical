...
public class HealthCheckFormController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<ActionResult<HealthCheckFormDTO>> CreateHealthCheckForm(
        HealthCheckFormDTO formDto
    )
    {
        ...
        {
            _logger.LogWarning(
                "Invalid model state for HealthCheckForm creation: {Errors}",
                string.Join(
                    ", ",
        ...
        try
        {
            _logger.LogInformation(
                "Creating HealthCheckForm with Title: {Title}",
                formDto.Title ?? "Unknown"
            );
            ...
            _logger.LogInformation(
                "HealthCheckForm created successfully with ID: {FormId}",
                createdForm.FormId
            );
        ...
        catch (InvalidOperationException ex)
        {
            _logger.LogError(
                ex,
                "Error creating HealthCheckForm with Title: {Title}",
                formDto.Title ?? "Unknown"
            );
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Unexpected error creating HealthCheckForm with Title: {Title}",
                formDto.Title ?? "Unknown"
            );
            return StatusCode(
                500,
                new { error = "An unexpected error occurred while creating the health check form." }
            );
        }
    }
    ...
    public async Task<ActionResult<IEnumerable<HealthCheckFormDTO>>> GetHealthCheckFormsByStudentId(
        int studentId
    )
    {
    ...
    public async Task<ActionResult<IEnumerable<HealthCheckFormDTO>>> GetHealthCheckFormsByParentId(
        int parentId
    )
    {
    ...
    public async Task<ActionResult<IEnumerable<HealthCheckFormDTO>>> GetHealthCheckFormsByStatus(
        string status
    )
    {
        try
    ...
    public async Task<ActionResult<IEnumerable<HealthCheckFormDTO>>> GetHealthCheckSchedules()
    {
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(
                500,
                new { error = "An error occurred while retrieving health check schedules." }
            );
        }
    }
    ...
    public async Task<ActionResult<HealthCheckFormDTO>> GetHealthCheckScheduleById(int id)
    {
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(
                500,
                new { error = "An error occurred while retrieving the health check schedule." }
            );
        }
    }

    [HttpPost("schedules")]
    public async Task<ActionResult<HealthCheckFormDTO>> CreateHealthCheckSchedule(
        HealthCheckFormDTO scheduleDto
    )
    {
        if (!ModelState.IsValid)
        {
            ...
            return BadRequest(
                new
                {
                    ...
                    details = ModelState
                        ...
                        .ToDictionary(
                            k => k.Key,
                            v => v.Value?.Errors?.Select(e => e.ErrorMessage) ?? new List<string>()
                        ),
                }
            );
        }
        ...
        {
            _logger.LogInformation(
                "Creating HealthCheckSchedule with Title: {Title}",
                scheduleDto.Title ?? "Unknown"
            );
            _logger.LogInformation(
            ...
            var createdSchedule = await _healthCheckFormService.CreateHealthCheckScheduleAsync(
                schedule
            );
            ...
            _logger.LogInformation(
                "HealthCheckSchedule created successfully with ID: {FormId}",
        ...
        catch (InvalidOperationException ex)
        {
            _logger.LogError(
                ex,
                "Error creating HealthCheckSchedule with Title: {Title}",
                scheduleDto.Title ?? "Unknown"
            );
            return BadRequest(new { error = ex.Message });
        }
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Unexpected error creating HealthCheckSchedule with Title: {Title}",
                scheduleDto.Title ?? "Unknown"
            );
            return StatusCode(
                500,
                new
                {
                    error = "An unexpected error occurred while creating the health check schedule.",
                }
            );
        }
    }
    ...
    public async Task<IActionResult> UpdateHealthCheckSchedule(
        int id,
        HealthCheckFormDTO scheduleDto
    )
    {
        ...
        {
            _logger.LogWarning(
                "ID mismatch - URL ID: {UrlId}, DTO FormId: {DtoFormId}",
                id,
                scheduleDto.FormId
            );
            return BadRequest("ID mismatch");
        ...
        try
        {
            var schedule = _mapper.Map<HealthCheckForm>(scheduleDto);
            _logger.LogInformation(
                "Mapped schedule - FormId: {FormId}, Status: {Status}",
                schedule.FormId,
                schedule.Status
            );
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(
                ...
                new
                {
                    error = "An unexpected error occurred while updating the health check schedule.",
                }
            );
        }
    }
