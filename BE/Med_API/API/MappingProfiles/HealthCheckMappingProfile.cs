...
public class HealthCheckMappingProfile : Profile
{
    public HealthCheckMappingProfile()
    {
        CreateMap<HealthCheckForm, HealthCheckFormDTO>()
            ...
            .ForMember(
                dest => dest.ConfirmedByStaff,
                opt => opt.MapFrom(src => src.ConfirmedByStaff)
            )
            ...
            .ForMember(
                ...
                opt =>
                    opt.MapFrom(src =>
                        src.StartTime.HasValue ? src.StartTime.Value.ToString(@"hh\:mm\:ss") : null
                    )
            );
        ...
        CreateMap<HealthCheckResult, HealthCheckResultDTO>()
            ...
            .ForMember(dest => dest.Student, opt => opt.MapFrom(src => src.Student))
            .ForMember(
                dest => dest.ExaminedByStaff,
                opt => opt.MapFrom(src => src.ExaminedByStaff)
            );
