 ...
 */
export const calculateTotalDuration = (checkItems, healthCheckItems, sessions) => {
 ...
 */
export const calculateResourceRequirements = (totalStudents, checkItems, healthCheckItems) => {
 ...
 */
export const generateHealthCheckNotificationTemplate = (checkItems, healthCheckItems, formData) => {
    const selectedItems = checkItems.map(itemId => {
        const item = healthCheckItems.find(h => h.id === itemId);
        return item?.name;
    }).filter(Boolean);
...
export const checkScheduleConflicts = (formData, existingSchedules = []) => {
    ...
    const totalStudents = formData.targetGrades ? formData.targetGrades.length * 25 : 0;
...
export const validateFormStep = (step, formData) => {
    ...
    switch (step) {
        ...
        case 2: // Health Check Items
            ...
            if (!formData.abnormalityProtocol?.trim()) {
                errors.abnormalityProtocol = "Quy trình xử lý bất thường là bắt buộc";
            }
            break;
