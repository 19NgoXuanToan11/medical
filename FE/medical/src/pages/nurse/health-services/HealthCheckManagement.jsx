...
const HealthCheckManagement = () => {
    ...
    const filteredHealthChecks = healthChecks.filter((healthCheck) => {
        ...
        if (activeSubTab === "scheduled" && healthCheck.status !== "scheduled")
            return false;
        if (activeSubTab === "active" && healthCheck.status !== "active") return false;
    ...
    const getStats = () => {
        ...
        const totalStudents = healthChecks.reduce((sum, healthCheck) => {
            if (healthCheck.status === "scheduled" || healthCheck.status === "active") {
                    ...
                    <div className="mb-4">
                        ...
                        <div className="w-full bg-neutral-200 dark:bg-neutral-700 rounded-full h-2">
                            <div
                                ...
                                style={{
                                    width: `${Math.round(
                                        (healthCheck.confirmedParents / healthCheck.totalStudents) * 100
                                    )}%`,
                                }}
                            ></div>
                        </div>
                    ...
                    <div className="mb-4">
                        ...
                        <div className="w-full bg-neutral-200 dark:bg-neutral-700 rounded-full h-2">
                            <div
                                ...
                                style={{
                                    width: `${Math.round(
                                        (healthCheck.completedStudents / healthCheck.totalStudents) * 100
                                    )}%`,
                                }}
                            ></div>
                        </div>
    ...
    const renderEmptyState = () => (
        <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-neutral-900 dark:text-neutral-100 mb-2">
                {activeSubTab === "scheduled" && "Không có khám sức khỏe nào được lên lịch"}
                {activeSubTab === "active" && "Không có khám sức khỏe nào đang thực hiện"}
                {activeSubTab === "completed" && "Chưa có khám sức khỏe nào hoàn thành"}
            </h3>
            <p className="text-neutral-500 dark:text-neutral-400 mb-6">
                {activeSubTab === "scheduled" && "Tạo lịch khám sức khỏe mới để bắt đầu"}
                {activeSubTab === "active" && "Các khám sức khỏe đang thực hiện sẽ hiển thị ở đây"}
                {activeSubTab === "completed" && "Lịch sử các khám sức khỏe đã hoàn thành sẽ hiển thị ở đây"}
            </p>
                        ...
                        {[
                            {
                                ...
                                label: "Đã lên lịch",
                                icon: FiCalendar,
                                count: healthChecks.filter((h) => h.status === "scheduled").length,
                            },
                            {
                                id: "active",
                            ...
                            },
                            {
                                ...
                                label: "Đã hoàn thành",
                                icon: FiCheckCircle,
                                count: healthChecks.filter((h) => h.status === "completed").length,
                            },
                        ].map((tab) => (
                            <button
                                key={tab.id}
                                onClick={() => setActiveSubTab(tab.id)}
                                className={`relative flex items-center px-4 py-3 text-sm font-medium transition-colors duration-200 ${activeSubTab === tab.id
