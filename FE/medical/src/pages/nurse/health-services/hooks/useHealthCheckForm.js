...
export const useHealthCheckForm = () => {
    ...
    const resourceReqs = calculateResourceRequirements(totalStudents, formData.checkItems, healthCheckItems);

    // Auto-calculate end time when start time or duration changes
    useEffect(() => {
        if (formData.scheduledTime && formData.estimatedDuration) {
            const endTime = calculateEndTime(formData.scheduledTime, formData.estimatedDuration);
    ...
    useEffect(() => {
        if (formData.checkItems.length > 0 && healthCheckItems.length > 0) {
            ...
            setFormData(prev => ({
                ...
                parentNotificationMessage: generateHealthCheckNotificationTemplate(formData.checkItems, healthCheckItems, formData)
            }));
        }
    ...
    }, [formData.scheduledDate, formData.scheduledTime, formData.endTime, formData.location, formData.equipmentNeeded]);
    ...
    const handleGradeSelection = useCallback((gradeId) => {
        setFormData(prev => {
            ...
            return {
                ...
                requiresApproval: totalStudents > 100 || prev.urgencyLevel === 'urgent',
    ...
    const handleCheckItemToggle = useCallback((itemId) => {
        setFormData(prev => {
