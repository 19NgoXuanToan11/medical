...
const HealthServicesManagement = () => {
  ...
  const [serviceFilter, setServiceFilter] = useState("all"); // all, vaccination, health_check
      ...
      setTimeout(() => {
        setHealthServices([
          ...
          // Active Services
          {
            ...
            checkItems: ["Chiều cao", "Cân nặng", "Thị lực", "Răng miệng", "Tim mạch", "Phổi"],
            startTime: "08:00",
  ...
  const filteredServices = healthServices.filter(service => {
    // Filter by tab
    if (activeTab === "scheduled" && service.status !== "scheduled") return false;
    if (activeTab === "active" && service.status !== "active") return false;
    if (activeTab === "completed" && service.status !== "completed") return false;

    // Filter by service type
    if (serviceFilter !== "all" && service.type !== serviceFilter) return false;
  ...
  const getStats = () => {
    const scheduledCount = healthServices.filter(s => s.status === "scheduled").length;
    const activeCount = healthServices.filter(s => s.status === "active").length;
    const completedThisMonth = healthServices.filter(s => {
    ...
    const totalStudents = healthServices.reduce((sum, service) => {
    ...
    setHealthServices(prev =>
      prev.map(service =>
        service.id === serviceId
          ? { ...service, status: "active", startTime: new Date().toISOString() }
          : service
      )
    );
  };
  ...
  const handleCompleteService = (serviceId) => {
    setHealthServices(prev =>
      prev.map(service =>
        service.id === serviceId
          ? { ...service, status: "completed", completedDate: new Date().toISOString() }
          : service
      )
    );
  };
        ...
        {service.status === "scheduled" && (
          <div className="mb-4">
            <div className="flex justify-between text-sm text-neutral-600 dark:text-neutral-400 mb-2">
              ...
              <span>{service.confirmedParents}/{service.totalStudents}</span>
            </div>
            <div className="w-full bg-neutral-200 dark:bg-neutral-700 rounded-full h-2">
              <div
                className="bg-blue-600 dark:bg-blue-500 h-2 rounded-full transition-all duration-300"
                style={{
                  width: `${Math.round((service.confirmedParents / service.totalStudents) * 100)}%`
                }}
              ></div>
            </div>
          </div>
        ...
        {service.status === "active" && (
          <div className="mb-4">
            <div className="flex justify-between text-sm text-neutral-600 dark:text-neutral-400 mb-2">
              ...
              <span>{service.completedStudents}/{service.totalStudents}</span>
            </div>
            <div className="w-full bg-neutral-200 dark:bg-neutral-700 rounded-full h-2">
              <div
                className="bg-amber-600 dark:bg-amber-500 h-2 rounded-full transition-all duration-300"
                style={{
                  width: `${Math.round((service.completedStudents / service.totalStudents) * 100)}%`
                }}
              ></div>
            </div>
          </div>
        ...
        {service.status === "completed" && (
          <div className="mb-4">
            <div className="flex justify-between text-sm mb-2">
              <span className="text-neutral-600 dark:text-neutral-400">Kết quả hoàn thành</span>
              <span className="text-green-600 dark:text-green-400 font-semibold">
                {service.vaccinatedStudents || service.completedStudents}/{service.totalStudents}
              </span>
            </div>
  ...
  const renderEmptyState = () => (
    <div className="text-center py-12">
      <div className="mx-auto h-12 w-12 text-neutral-400 dark:text-neutral-500 mb-4">
        {activeTab === "scheduled" && <FiCalendar className="w-12 h-12" />}
        {activeTab === "active" && <FiClock className="w-12 h-12" />}
        {activeTab === "completed" && <FiCheckCircle className="w-12 h-12" />}
      </div>
      <h3 className="text-lg font-medium text-neutral-900 dark:text-neutral-100 mb-2">
        {activeTab === "scheduled" && "Không có dịch vụ nào được lên lịch"}
        {activeTab === "active" && "Không có dịch vụ nào đang thực hiện"}
        {activeTab === "completed" && "Chưa có dịch vụ nào hoàn thành"}
      </h3>
      <p className="text-neutral-500 dark:text-neutral-400 mb-6">
        {activeTab === "scheduled" && "Tạo lịch dịch vụ y tế mới để bắt đầu"}
        {activeTab === "active" && "Các dịch vụ đang thực hiện sẽ hiển thị ở đây"}
        {activeTab === "completed" && "Lịch sử các dịch vụ đã hoàn thành sẽ hiển thị ở đây"}
      </p>
      {activeTab === "scheduled" && (
        <Link
    ...
    <div>
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-neutral-600 dark:text-neutral-400 mt-1">
              Quản lý tiêm chủng và khám sức khỏe định kỳ của học sinh
            </p>
          </div>
          <Link
            to="/nurse/health-services/create"
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 rounded-xl p-6 border border-blue-200 dark:border-blue-800">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-blue-600 dark:text-blue-400 text-sm font-medium">Đã lên lịch</p>
        ...
        <div className="bg-gradient-to-br from-amber-50 to-amber-100 dark:from-amber-900/20 dark:to-amber-800/20 rounded-xl p-6 border border-amber-200 dark:border-amber-800">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-amber-600 dark:text-amber-400 text-sm font-medium">Đang thực hiện</p>
        ...
        <div className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 rounded-xl p-6 border border-green-200 dark:border-green-800">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-green-600 dark:text-green-400 text-sm font-medium">Hoàn thành tháng này</p>
        ...
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 rounded-xl p-6 border border-purple-200 dark:border-purple-800">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-purple-600 dark:text-purple-400 text-sm font-medium">Tổng học sinh</p>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex items-center space-x-4">
            ...
            <select
              ...
              className="px-4 py-2 border border-neutral-300 dark:border-neutral-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent bg-white dark:bg-neutral-700 text-neutral-900 dark:text-neutral-100"
            >
      ...
      <div className="bg-white dark:bg-neutral-800 rounded-lg shadow-md border border-neutral-200 dark:border-neutral-700 overflow-hidden">
        <div className="border-b border-neutral-200 dark:border-neutral-700">
          <nav className="flex">
            {[
              { id: "scheduled", label: "Đã lên lịch", icon: FiCalendar, count: stats.scheduled },
              { id: "active", label: "Đang thực hiện", icon: FiClock, count: stats.active },
              { id: "completed", label: "Đã hoàn thành", icon: FiCheckCircle }
            ].map((tab) => (
              <button
                key={tab.id}
