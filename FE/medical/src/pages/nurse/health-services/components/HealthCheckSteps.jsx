...
import {
    FiUser, FiClipboard, FiUsers, FiEye,
    FiArrowLeft, FiArrowRight, FiSave, FiX, FiCheck
} from 'react-icons/fi';
        ...
        <div className="flex items-center justify-center mb-8">
            {healthCheckStepsConfig.map((step, index) => (
                <div key={step.step} className="flex items-center">
                    <div
                        className={`flex items-center justify-center w-12 h-12 rounded-full cursor-pointer transition-all duration-300 ${currentStep === step.step
                    ...
                    <div className="ml-3 text-left">
                        <p className={`text-sm font-medium ${currentStep === step.step
                        ...
                        <p className={`text-xs ${currentStep === step.step
                    ...
                    {index < healthCheckStepsConfig.length - 1 && (
                        <div className={`w-8 h-0.5 mx-4 ${currentStep > step.step ? 'bg-success-500 dark:bg-success-400' : 'bg-neutral-300 dark:bg-neutral-600'
                            }`} />
                    )}
                </div>
            ))}
...
export const ProgressBar = ({ currentStep }) => {
    const progress = ((currentStep - 1) / (healthCheckStepsConfig.length - 1)) * 100;
        ...
        <div className="w-full bg-neutral-200 dark:bg-neutral-700 rounded-full h-2 mb-6">
            ...
            <div className="text-center">
                <span className="text-sm text-neutral-600 dark:text-neutral-400">
                    Tiến độ: {currentStep}/{healthCheckStepsConfig.length} bước ({Math.round(progress)}%)
                </span>
            </div>
        </div>
    );
...
export const StepHeader = ({ currentStep, validationErrors = {}, scheduleConflicts = [] }) => {
    const currentStepConfig = healthCheckStepsConfig.find(step => step.step === currentStep);
    ...
    return (
        <div className="text-center mb-8">
            ...
            {scheduleConflicts.length > 0 && (
                <div className={`border rounded-lg p-4 mb-4 max-w-2xl mx-auto text-left ${hasCriticalConflicts
